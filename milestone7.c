...
int isInQueue(int queue[MAX_TRAVELERS], int count, int travelerIndex) {
    for (int i = 0; i < count; i++) {
        if (queue[i] == travelerIndex) return 1;
    }
    return 0;
}

void addToQueue(int queue[MAX_TRAVELERS], int *count, int travelerIndex) {
    if (*count >= MAX_TRAVELERS) return;
    if (!isInQueue(queue, *count, travelerIndex)) {
        queue[*count] = travelerIndex;
        (*count)++;
    }
}

void removeFromQueue(int queue[MAX_TRAVELERS], int *count, int travelerIndex) {
    for (int i = 0; i < *count; i++) {
        if (queue[i] == travelerIndex) {
...
int chooseNextTraveler(int queue[MAX_TRAVELERS], int count,
...
char scheduler[10];
strcpy(scheduler, argv[2]);

if (strcmp(scheduler, "fcfs") != 0 && strcmp(scheduler, "sjf") != 0) {
...
FILE *file = fopen(argv[3], "r");
    ...
    int waitingQueues[MAX_NODES][MAX_TRAVELERS];
    int waitingCounts[MAX_NODES] = {0};
