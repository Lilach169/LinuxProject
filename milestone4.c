...
void dijkstra(int node, int graph[MAX_NODES][MAX_NODES], int source, int destination, int path[MAX_NODES], int *pathCount) {
    int dist[MAX_NODES];
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < travelersCount; i++) {
        if (!readInt(file, &travelers[i].source) || !readInt(file, &travelers[i].destination)) {
        ...
        dijkstra(node, graph, travelers[i].source, travelers[i].destination,
                 travelers[i].path, &travelers[i].pathCount);
    }
    ...
    InitWindow(800, 600, "Milestone 4 - Multiple Travelers");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
            ...
            for (int i = 0; i < travelersCount; i++) {
                ...
                if (t->waiting) {
                    ...
                    if (t->timer >= TICK_TIME) {
                        ...
                        if (t->currentStep >= weight) {
                            t->pathIndex++;
                            t->currentStep = 0;

                            if (t->pathIndex < t->pathCount - 1) {
                                t->waiting = 1;
                            } else {
                                t->finished = 1;
                                kill(t->pid, SIGTERM);
        ...
        for (int i = 0; i < node; i++) {
            for (int j = 0; j < node; j++) {
                if (graph[i][j] != 0) {
                    ...
                    DrawText(TextFormat("%d", graph[i][j]), labelX, labelY, 20, WHITE);
                }
            }
        }
        ...
        for (int i = 0; i < node; i++) {
            ...
            DrawText(text, pos[i].x - width / 2, pos[i].y - 11, 22, BLACK);
        }
        ...
        for (int i = 0; i < travelersCount; i++) {
            ...
            if (t->finished || t->pathIndex >= t->pathCount - 1) {
                ...
                float move = (float)t->currentStep / weight;
            ...
            DrawCircleV(travelerPos, 12, t->color);
            DrawText(TextFormat("T%d", i + 1), travelerPos.x - 10, travelerPos.y - 30, 18, t->color);
        }
